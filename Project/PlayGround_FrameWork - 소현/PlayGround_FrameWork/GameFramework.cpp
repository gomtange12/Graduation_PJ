...
void CGameFramework::ProcessInput()
{
	...
	{
		DWORD dwDirection = 0;
		if KEYDOWN('W') dwDirection |= DIR_FORWARD;
		if (KEYDOWN('S')) dwDirection |= DIR_BACKWARD;
		if (KEYDOWN('A')) dwDirection |= DIR_LEFT;
		if (KEYDOWN('D')) dwDirection |= DIR_RIGHT;
		if (KEYDOWN('Q')) dwDirection |= DIR_UP;
		if (KEYDOWN('E')) dwDirection |= DIR_DOWN;
