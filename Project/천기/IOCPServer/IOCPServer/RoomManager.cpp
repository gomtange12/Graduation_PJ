			...
			for (int j = 0; j < SOLO_NUM; ++j) { //아이디를 넣어줌
				if (room[i]->m_ids[j] < 0) {
					if (room[i]->m_ids[j] != id) {
						...
						{
							room[i]->m_full = true;
							dynamic_cast<TimerThread*>(THREADMANAGER->FindThread(TIMER_TH))->AddTimer(room[i]->m_ids[0], OP_ALLPOS, RNumber, GetTickCount() + 4000);
							...
							objectManager->GetPlayer(room[i]->m_ids[0])->m_xmf3Position = XMFLOAT3(-530, 50, 745);
