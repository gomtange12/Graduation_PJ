...
void CScene::CheckObjectByObjectCollisions() {
	//Ã¼Å©´ÂµÇ´ÂµíÇÑµ¥ ¿Ö Áß´ÜÁ¡ÀÌ ¾ÈµÇÁö?
	...
	case CONCERTMAP:
		...
		if (m_ppConcertObjects[0])
		{
			...
			if (m_ppConcertObjects[0]->GetBoundingBox().Contains(PLAYER->GetOtherPlayer()->GetBoundingBox()))
			{
				//std::cout << "ÀÍ½ºÅÙÆ® °öÇÏ°ÔÇÏ±â" << m_ppConcertObjects[0]->GetBoundingBox().Extents.y * objScale * 100 << std::endl;
				//std::cout << "¿ÍÀÌ°ª" << m_ppConcertObjects[0]->GetBoundingBox().Extents.y + m_ppConcertObjects[0]->GetBoundingBox().Center.y << std::endl;
				//std::cout << "ÇÃ·¹ÀÌ¾î ¼¾ÅÍ°ª" << PLAYER->GetOtherPlayer()->GetBoundingBox().Center.y - PLAYER->GetOtherPlayer()->GetBoundingBox().Extents.y + 10 << std::endl;
				//ÀÌ ¾È¿¡ ¿ÀºêÁ§Æ® Ãæµ¹À» ³Ö¾î¾ßÇÒ¼öµµ
