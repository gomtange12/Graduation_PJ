...
void ObjManager::MovePkt(int id, unsigned char *packet)
{
	...
	PACKETMANAGER->MovePacket(xmf3Shift,id);
...
void ObjManager::MoveUpdate(int id)
{
	Player* player = g_clients[id];
	...
	XMFLOAT3 xmf3Velocity = Vector3::ScalarProduct(player->m_xmf3Velocity, 0.02, false);
	...
	float fDeceleration = (player->m_fFriction * 0.02);
