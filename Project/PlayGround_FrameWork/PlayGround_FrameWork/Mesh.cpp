...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 
CHeightMapImage::CHeightMapImage(LPCTSTR pFileName, int nWidth, int nLength, XMFLOAT3 xmf3Scale, int terrainID)
{
	m_nWidth = nWidth;
	m_nLength = nLength;
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, int meshterrainID, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void *pContext) : CMesh(pd3dDevice, pd3dCommandList)
{
...
//	m_nStride = sizeof(CTexturedVertex);
	...
	XMFLOAT4 xmf4HighColor(0.8f, 0.3f, 0.8f, 1.0f);
	XMFLOAT4 xmf4RedColor(0.8f, 0.0f, 0.0f, 1.0f);

	m_meshterrainID = meshterrainID;
//	CTexturedVertex *pVertices = new CTexturedVertex[m_nVertices];
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			//pVertices[i].m_xmf4Diffuse = Vector4::Add(OnGetColor(x, z, pContext), xmf4Color); GPU에게 넘기는 색
			pVertices[i].m_xmf2TexCoord = XMFLOAT2(float(x) / float(cxHeightMap - 1), float(czHeightMap - 1 - z) / float(czHeightMap - 1));
			pVertices[i].m_xmf2TexCoord2 = XMFLOAT2(float(x) / float(m_xmf3Scale.x*0.5f), float(z) / float(m_xmf3Scale.z*0.5));
			//for (int xcell = 0; xcell < cxHeightMap; ++xcell)
			//{
			//	for (int zcell = 0; zcell < czHeightMap; ++zcell)
			//	{
			//		if (meshterrainID == 1)
			//		{
			//			if (OnGetColor(x, z, pContext).x < xmf4HighColor.x)
			//			{
			//				//pVertices[i].m_xmf4Diffuse = Vector4::Add(OnGetColor(x, z, pContext), xmf4RedColor);
			//				pVertices[i].m_xmf4Diffuse = Vector4::Add(OnGetColor(x, z, pContext), xmf4RedColor);
			//			}
			//		}
			//	}
			...
			//else
			{
				...
				if (125.f < OnGetHeight(x, z, pContext))
				{
					pVertices[i].m_xmf4Diffuse = Vector4::Add(OnGetColor(x, z, pContext), xmf4HighColor);
					/*pVertices[i].m_xmf4Diffuse.x = m_mapcolor[i].x;
					pVertices[i].m_xmf4Diffuse.y = m_mapcolor[i].y;
					pVertices[i].m_xmf4Diffuse.z = m_mapcolor[i].z;
					pVertices[i].m_xmf4Diffuse.w = m_mapcolor[i].w;*/
...
XMFLOAT4 CHeightMapGridMesh::OnGetColorByHeight(int x, int z, void * pContext)
{
	XMFLOAT3 xmf3LightDirection = XMFLOAT3(-1.0f, 1.0f, 1.0f);
	xmf3LightDirection = Vector3::Normalize(xmf3LightDirection);
	CHeightMapImage *pHeightMapImage = (CHeightMapImage *)pContext;
	XMFLOAT3 xmf3Scale = pHeightMapImage->GetScale();
	XMFLOAT4 xmf4IncidentLightColor(0.9f, 0.9f, 0.9f, 0.5f);
	float fScale = Vector3::DotProduct(pHeightMapImage->GetHeightMapNormal(x, z), xmf3LightDirection);
	fScale += Vector3::DotProduct(pHeightMapImage->GetHeightMapNormal(x + 1, z), xmf3LightDirection);
	fScale += Vector3::DotProduct(pHeightMapImage->GetHeightMapNormal(x + 1, z + 1), xmf3LightDirection);
	fScale += Vector3::DotProduct(pHeightMapImage->GetHeightMapNormal(x, z + 1), xmf3LightDirection);
