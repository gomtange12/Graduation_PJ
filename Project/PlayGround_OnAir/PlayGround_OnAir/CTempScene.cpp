...
ID3D12DescriptorHeap *CTempScene::m_pd3dCbvSrvDescriptorHeap = NULL;

D3D12_CPU_DESCRIPTOR_HANDLE	CTempScene::m_d3dCbvCPUDescriptorStartHandle;
D3D12_GPU_DESCRIPTOR_HANDLE	CTempScene::m_d3dCbvGPUDescriptorStartHandle;
D3D12_CPU_DESCRIPTOR_HANDLE	CTempScene::m_d3dSrvCPUDescriptorStartHandle;
D3D12_GPU_DESCRIPTOR_HANDLE	CTempScene::m_d3dSrvGPUDescriptorStartHandle;
							
D3D12_CPU_DESCRIPTOR_HANDLE	CTempScene::m_d3dCbvCPUDescriptorNextHandle;
D3D12_GPU_DESCRIPTOR_HANDLE	CTempScene::m_d3dCbvGPUDescriptorNextHandle;
D3D12_CPU_DESCRIPTOR_HANDLE	CTempScene::m_d3dSrvCPUDescriptorNextHandle;
D3D12_GPU_DESCRIPTOR_HANDLE	CTempScene::m_d3dSrvGPUDescriptorNextHandle;
CTempScene::CTempScene()
{
}
...
void CTempLoadScene::UpdateShaderVariables(ID3D12GraphicsCommandList * pd3dCommandList)
{
}

CTempInGameScene::CTempInGameScene()
{
}
...
void CTempInGameScene::UpdateShaderVariables(ID3D12GraphicsCommandList * pd3dCommandList)
{
}

CTempGameOverScene::CTempGameOverScene()
{
}
...
void CTempGameOverScene::UpdateShaderVariables(ID3D12GraphicsCommandList * pd3dCommandList)
{
}

CTempMenuScene::CTempMenuScene()
{
}
...
void CTempMenuScene::UpdateShaderVariables(ID3D12GraphicsCommandList * pd3dCommandList)
{
}
bool CTempScene::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	return(false);
}

bool CTempScene::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	switch (nMessageID)
	{
	case WM_KEYDOWN:
...
void CTempScene::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(TEMPLIGHTS) + 255) & ~255); //256ÀÇ ¹è¼ö
	m_pd3dcbLights = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);

	m_pd3dcbLights->Map(0, NULL, (void **)&m_pcbMappedLights);
}

void CTempScene::BuildDefaultLightsAndMaterials()
{
	m_nLights = 4;
	m_pLights = new TEMPLIGHT[m_nLights];
	::ZeroMemory(m_pLights, sizeof(TEMPLIGHT) * m_nLights);

	m_xmf4GlobalAmbient = XMFLOAT4(0.15f, 0.15f, 0.15f, 1.0f);

	m_pLights[0].m_bEnable = true;
	m_pLights[0].m_nType = TEMP_POINT_LIGHT;
	...
	m_pLights[1].m_nType = TEMP_SPOT_LIGHT;
	...
	m_pLights[2].m_nType = TEMP_DIRECTIONAL_LIGHT;
	m_pLights[2].m_xmf4Ambient = XMFLOAT4(0.3f, 0.3f, 0.3f, 1.0f);
	m_pLights[2].m_xmf4Diffuse = XMFLOAT4(0.7f, 0.7f, 0.7f, 1.0f);
	m_pLights[2].m_xmf4Specular = XMFLOAT4(0.4f, 0.4f, 0.4f, 0.0f);
	m_pLights[2].m_xmf3Direction = XMFLOAT3(1.0f, 0.0f, 0.0f);
	m_pLights[3].m_bEnable = true;
	m_pLights[3].m_nType = TEMP_SPOT_LIGHT;
...
void CTempScene::ReleaseUploadBuffers()
{
...
bool CTempScene::ProcessInput(UCHAR * pKeysBuffer)
{
	return false;
}

void CTempScene::Render(ID3D12GraphicsCommandList * pd3dCommandList, std::shared_ptr<CCamera> pCamera)
{
...
void CTempScene::ReleaseObjects()
{
...
//void CTempScene::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
//{
//	/*::memcpy(m_pcbMappedLights->m_pLights, m_pLights, sizeof(TEMPLIGHT) * m_nLights);
//	::memcpy(&m_pcbMappedLights->m_xmf4GlobalAmbient, &m_xmf4GlobalAmbient, sizeof(XMFLOAT4));
//	::memcpy(&m_pcbMappedLights->m_nLights, &m_nLights, sizeof(int));*/
//}

void CTempScene::ReleaseShaderVariables()
{
	if (m_pd3dcbLights)
	{
...
void CTempScene::BuildObjects(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList)
{
...
D3D12_GPU_DESCRIPTOR_HANDLE CTempScene::CreateConstantBufferViews(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, int nConstantBufferViews, ID3D12Resource * pd3dConstantBuffers, UINT nStride)
{
...
D3D12_GPU_DESCRIPTOR_HANDLE CTempScene::CreateShaderResourceViews(ID3D12Device * pd3dDevice, CTexture * pTexture, UINT nRootParameter, bool bAutoIncrement)
{
...
void CTempScene::AnimateObjects(float fTimeElapsed)
{
...
void CTempScene::CreateCbvSrvDescriptorHeaps(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, int nConstantBufferViews, int nShaderResourceViews)
{
