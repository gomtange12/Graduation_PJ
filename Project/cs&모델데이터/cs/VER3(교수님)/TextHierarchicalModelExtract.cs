...
public class TextHierarchicalModelExtract : MonoBehaviour
{
   ...
   void WriteString(int nLevel, string strToWrite)
    ...
    void WriteAnimationClipInfo(int nLevel)
    {
        ...
        for (int j = 0; j < m_raAnimationClips.Length; j++)
        {
            ...
            int nKeyFrame = 0;
            while ((fKeyFrameTime < m_raAnimationClips[j].length) && (nKeyFrame < nKeyFrames))
            {
                m_raAnimationClips[j].SampleAnimation(gameObject, fKeyFrameTime);

                WriteAnimationTransforms(nLevel+2, "<Transforms>: " + nKeyFrame + " " + fKeyFrameTime + " ");
                nKeyFrame++;
    ...
    void Start()
    {
        ...
        if (m_raAnimationClips.Length > 0) WriteAnimationClipInfo(0);
